...
PurePursuit::PurePursuit()
  : RADIUS_MAX_(9e10) // 最大运动半径
  , KAPPA_MIN_(1 / RADIUS_MAX_) // 最小运动曲率
  , is_linear_interpolation_(false) //  是否线性插值
  , next_waypoint_number_(-1) // 下一个目标路径点 在发布轨迹列中的下标 
  , lookahead_distance_(0)
  , minimum_lookahead_distance_(6)
  , current_linear_velocity_(0) // 当前车辆速度
  ...
  int next_waypoint, geometry_msgs::Point* next_target) const
{
  ...
  // 如果是最后一个轨迹点，则将最后一个轨迹点赋值给 *next_target 然后跳出这个函数
...
void PurePursuit::getNextWaypoint()
{
  ...
  for (int i = 0; i < path_size; i++)
  {
    ...
    if (i == (path_size - 1))
    {
      ...
      next_waypoint_number_ = i - path_size +1;
      return;
    }
