...
void PurePursuitNode::initForROS()
{
  ...
  private_nh_.param("is_circle", is_circle, false);
  ...
  const bool& can_get_curvature, const double& kappa) const
{
  ...
  ccs.cmd.linear_velocity = can_get_curvature ? computeCommandVelocity() : 3;
  ccs.cmd.linear_acceleration = can_get_curvature ? computeCommandAccel() : 0.5;
  ...
  const autoware_msgs::LaneConstPtr& msg)
{
  ...
  a = getPlaneDistance(msg->waypoints.at(0).pose.pose.position,msg->waypoints.at(size).pose.pose.position) < 3 ? 1:0;
